  ...
  useEffect(() => {
    AsyncStorage.getItem('stickyNotificationId').then((value) => {
      if (value !== null) {
        ...
        AsyncStorage.removeItem('stickyNotificationId');
      }
    });
  }, []);

  const handleLayout = (event) => {
    // set fixed height for the view (Prevents the view from jumping when the keyboard is opened)
            ...
            AsyncStorage.getItem('stickyNotificationId').then((value) => {
              if (value === null) {
                Notifications.scheduleNotificationAsync({
                  ...
                  AsyncStorage.setItem('stickyNotificationId', JSON.stringify(notificationId));
                });
              }
            });
          }
